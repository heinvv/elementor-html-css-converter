...
import { VariablesImportExportModal } from '../components/VariablesImportExportModal';
import { ModalManager } from '../types/utils';

export const createVariablesModalManager = (): ModalManager => {
	...
	const manager: ModalManager = {
		...
		init() {
			...
			container.id = 'ehcc-variables-import-export-container';
			...
			try {
				...
				this.root.render(
					ReactLib.createElement(
						ErrorBoundary,
						{},
						ReactLib.createElement(VariablesImportExportModal, {
							isOpen: this.isOpen,
							onClose: () => this.close(),
							apiUrl: apiUrl,
						})
