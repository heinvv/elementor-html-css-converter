...
/**
 * Transform Property Converter
 *
 * Converts CSS transform properties to Elementor atomic widget format.
 * Supports: transform, transform-origin, perspective, perspective-origin
 *
...
class Transform_Converter extends Property_Converter_Base {
	...
	/**
	 * Maps CSS transform functions to internal function types.
	 */
	...
	private function parse_transform_functions( string $value ): array {
		...
		// Match transform functions like translateX(10px), rotate(45deg), etc.
	...
	private function parse_rotate_function( string $function_name, string $args ): ?array {
		...
		switch ( $function_name ) {
			...
			case 'rotatez':
				...
				break;
			case 'rotate3d':
				// rotate3d(x, y, z, angle) - only the angle matters for our purposes
	...
	private function parse_transform_origin( string $value ): ?array {
		// Transform origin is not fully implemented in css-converter
	...
	private function parse_perspective_origin( string $value ): ?array {
		// Perspective origin is not fully implemented in css-converter
		// Return null to skip this property
		return null;
	}
